...
fn test_repo() -> Result<()> {
    let key = str_rnd(12).to_ascii_lowercase();

    let forked_url = env::var("GR_BITBUCKET_FORKED_REPOSITORY_URL")
        .unwrap_or("git@bitbucket.org:grbin-test2/gr-bin-forked.git".to_string());
    // On Bitbucket you can only fork into an org, not to a user
    let fork_into = env::var("GR_BITBUCKET_FORK_INTO").unwrap_or("grbin-test".to_string());
    ...
    // Create a commit because Bitbucket can't
    fs::write("README.md", "")?;
    exec("git", vec!["add", "-A"], false)?;
    exec("git", vec!["commit", "-m", "README"], false)?;
